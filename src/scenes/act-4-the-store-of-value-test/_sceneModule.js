// Act IV — the scene-module contract, in one place.
//
// The same contract mechanics as Act I's, Act II's and Act III's
...
//   · A within-group handoff (the film's morph or the legacy's own boundary
//     crossfade) plays the incoming scene's morphIn; backward across the
//     boundary reconstructs the outgoing scene's end state instantly.

import { ensureStage, destroyStage, TOTAL_BUILDS, reducedMotion } from './_testStage.js';

export function makeSceneModule({ id, number, title, notes, entry, morphIn, transitions, onEnterExtra, onExitExtra }) {
  return {
    id,
    sceneGroup: 'act4-test',
    section: 'act iv — the store-of-value test',
    number,
    title,
