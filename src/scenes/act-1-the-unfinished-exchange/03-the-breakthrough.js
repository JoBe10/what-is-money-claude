// Scene 3 — The Breakthrough (9 beats). The heart of the film.
//
// TRANSPLANTED FROM THE CLOSED GATE: `src/proto/gate-2/s3-the-breakthrough.js`
// as judged, carrying the ruled birth and the ruled failure language.
//
// Scene 2 morphs into this scene — same stage, no cut: the binding line yields,
// the figures re-gather, the receded record's light goes home into the terminal
// dot, and the birth consumes the absence it leaves. The birth is the film's
// first orange and it is **pool** (presenter-ruled 26 August 2026); condense is
// the named banked alternate and both alternates stay runnable in the
// prototype, which is where the file-keeping clause keeps them. The claim's
// micro-motions — the pulse as the contraction completes, the breath at the
// release — are KEPT (R5, closed 28 August 2026).
//
...
// departs and the shoes arrive as distinct overlapping events, and at no point
// may the claim read as becoming the shoe. Beat 9 completes a full recorded
// handoff to clean black before the separation lines land (D4-A).
//
// Landed states — approved cells, by construction (states.json
// approvedSetCurrent): s3-b1-p2 · s3-b2 · s3-b3 · s3-b4-a · s3-b5-a ·
// s3-b6-a · s3-b7-b · s3-b8-a · s3-b9-a. Beat 1's remnant is the absence's —
// the carried birth composition with only the terminal dot the pool gathered
// from.

import { GEOM } from './_exchangeStage.js';
import { makeSceneModule } from './_sceneModule.js';
import { playBirth, applyPreBirth, fadeSubstrateIn } from './_birth.js';
...
// birth's composition; the receded record's light goes home — the unresolved
// half, gathered for what happens to it next — and the birth consumes it.
...
// The b5 record rises into the corridor as the birth's substrate. Under the
// ruled failure language the record is the absence's own — the service path on
// record and the terminal dot (s2-b5-b-p2) — and the lift drains its light
// home into the dot: the delivered half has said its piece, the service path's
// receded light dims out of the frame, and the terminal dot carries the
// gathered light to the patient's edge at the birth's height. No line exists
// before the birth, and no degraded stroke survives into any settled frame.
...
// Cold entry: the substrate assembles quickly — figures, the unresolved half —
// and the birth plays.
...
const transitions = {
  ...
  // The void's last light crosses from the dot's edge into the disc, the dot
  // lets go, and the claim answers with its micro-pulse — its first breath
  // (kept per R5, closed 28 Aug).
  1: (mod, stage) => {
    const tl = stage.timeline();
