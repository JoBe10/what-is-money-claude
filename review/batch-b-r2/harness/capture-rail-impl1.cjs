// Act II rail implementation — the capture strip, started at Session 1
// (docs/act-2-rail-impl-brief.md §2).
//
// The record of what the deck actually plays, out of the spliced film at
// 1920×1080: every settled state of Scenes 5–7 — all twenty-two — plus the
...
// the gate's criterion is now the rail's own continuity and continuity only
// exists between frames.
//
// Session 2 completes the strip with Scenes 8–10.
//
// Usage: node capture-rail-impl1.cjs [--port 5273]
...
const GESTURES = [
  ...
  ['g-s7s8-handover', 8, 0, 'boundary', 700,
    'The rail releasing as Scene 8 takes the frame — the seam Session 2 replaces.']
];
...
<html lang="en"><head><meta charset="utf-8"><title>Act II on the rail — the capture strip, Session 1</title>
...
<h1>Act II on the rail — the capture strip, Session 1</h1>
<p class="note">Scenes 5, 6 and 7 as the deck actually plays them, captured out of the spliced film at 1920&times;1080.
The settled frames are the ones the landed-state proof compares against the approved r2 cells — all
eighteen rail states at zero differing pixels. Session 2 completes the strip with Scenes 8, 9 and 10.</p>
${sceneSections}
${motionSection}
</body></html>`);

  fs.writeFileSync(path.join(__dirname, '..', 'strip-rail-impl1.json'), JSON.stringify({
