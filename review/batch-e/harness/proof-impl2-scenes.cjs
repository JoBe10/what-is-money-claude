// Batch E implementation, Session 2 — the landed-state proof for the new scenes
// (docs/batch-e-implementation-brief.md §3).
//
// One claim, made checkable: EVERY SETTLED STATE OF SCENES 28–30 IS ITS
// APPROVED CELL. Seven states — S28's three, S29's two, S30's two, the frozen
// beat map (master §13, 4 Sep 2026) — each mounted cold
...
const STATES = [
  ...[0, 1, 2].map((b) => [5, b, `s28-b${b + 1}`]),
  ...[0, 1].map((b) => [6, b, `s29-b${b + 1}`]),
  ...[0, 1].map((b) => [7, b, `s30-b${b + 1}`])
];
