// Batch D implementation, Session 2 — the landed-state proof for the new
// scenes, pre-splice (docs/batch-d-implementation-brief.md §3).
//
// EVERY SETTLED STATE OF SCENES 20–23 IS ITS APPROVED CELL. Nineteen states —
// S20's five, S21's five, S22's three (the merged map), S23's six — each
// mounted cold through the real engine in the scratch route (`?proto=act4`,
// slides 5–8) at 1920×1080 and compared per pixel against the archived cell
// PNG (review/act-4/states — the approved set) and against a fresh render of
// that cell through its own states pipeline. Three comparisons per state;
// any differing pixel (channel delta > 2/255) fails. The batch proof then
// repeats the claim for all 42 states through the spliced deck.
//
...
const STATES = [
  ...[0, 1, 2, 3, 4].map((b) => [5, b, `s20-b${b + 1}`]),
  ...[0, 1, 2, 3, 4].map((b) => [6, b, `s21-b${b + 1}`]),
  ...[0, 1, 2].map((b) => [7, b, `s22-b${b + 1}`]),
  ...[0, 1, 2, 3, 4, 5].map((b) => [8, b, `s23-b${b + 1}`])
];
...
(async () => {
  ...
  fs.writeFileSync(path.join(__dirname, '..', 'landed-proof-impl2.json'), JSON.stringify({
    ...
    session: 'batch-d-impl-2',
    rule: 'each of the 19 settled states of Scenes 20–23, mounted cold through the real engine in the scratch route (?proto=act4) at 1920×1080, is its approved cell (review/act-4/states — presenter-approved in full 3 Sep 2026) at zero differing pixels (channel delta > 2/255 counts). Comparisons: scene↔archived cell, fresh cell↔archived cell, scene↔fresh cell.',
