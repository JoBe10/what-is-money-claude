// Batch D implementation, Session 1 — the landed-state proof
// (docs/batch-d-implementation-brief.md §2).
//
// One claim, made checkable: EVERY SETTLED STATE OF SCENES 16–19 IS ITS
// APPROVED CELL. Twenty-three states — S16's eight, S17's five, S18's eight,
// S19's two — each mounted cold through the real engine in the scratch route
// (`?proto=act4`, where the scenes run in place until Session 2 splices them
// into the manifest) at 1920×1080 and compared per pixel against the
// archived cell PNG (review/act-4/states — the approved set, presenter-
// approved in full 3 Sep 2026), and against a fresh render of that cell
// through its own states pipeline.
//
...
const CELLS = path.join(REPO, 'review', 'act-4', 'states');

// [proto slide (1-based within ?proto=act4), build, cell id]
const STATES = [
  ...[0, 1, 2, 3, 4, 5, 6, 7].map((b) => [1, b, `s16-b${b + 1}`]),
  ...[0, 1, 2, 3, 4].map((b) => [2, b, `s17-b${b + 1}`]),
  ...[0, 1, 2, 3, 4, 5, 6, 7].map((b) => [3, b, `s18-b${b + 1}`]),
  ...[0, 1].map((b) => [4, b, `s19-b${b + 1}`])
];
...
(async () => {
  ...
  for (const [slide, build, id] of STATES) {
    await freshPage();
    await page.goto(`${BASE}/?proto=act4&slide=${slide}`, { waitUntil: 'networkidle' });
  ...
  fs.writeFileSync(path.join(__dirname, '..', 'landed-proof-impl1.json'), JSON.stringify({
    ...
    rule: 'each of the 23 settled states of Scenes 16–19, mounted cold through the real engine in the scratch route (?proto=act4) at 1920×1080, is its approved cell (review/act-4/states — presenter-approved in full 3 Sep 2026) at zero differing pixels (channel delta > 2/255 counts). Comparisons: scene↔archived cell, fresh cell↔archived cell, scene↔fresh cell.',
