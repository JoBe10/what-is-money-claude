// Batch D implementation, Session 1 — the FAST-scope smoke
// (docs/batch-d-implementation-brief.md §2).
//
// Six proofs, one run:
//   1. Zero deck impact: the deck boots unchanged — 38 slides (the count
//      since the Act IV foundation's ruled splice), the same first id, 0
//      console errors. The scenes run in place behind the existing count;
//      nothing is spliced until Session 2. (The two ruled changes at the
//      source — legacy 4-10's carrier keyed to build 2 and the stress
//      stage's `visible` — are slide-internal and change no count.)
//   2. Route boot: `?proto=act4` runs the four scenes through the real
//      engine — 23 states.
...
//   4. The three in-act boundaries, both directions, landings recorded.
//   5. Direct entry at every build (URL + restored build step — the
//      engine's own deep-link path), all settled.
//   6. Reduced-motion parity: the same 23 cold entries under
//      prefers-reduced-motion serialize IDENTICALLY to the settled
//      motion-on states (window.__act4's serializer) — checked
...
const SCENES = [
  { slide: 1, id: 'return-to-the-open-exchange', builds: 8 },
  { slide: 2, id: 'what-the-carrier-must-preserve', builds: 5 },
  { slide: 3, id: 'the-100-year-test', builds: 8 },
  { slide: 4, id: 'invert-the-question', builds: 2 }
];
const EXPECTED_DECK_SLIDES = 38;
...
(async () => {
  ...
  // ---- 4 · the three boundaries, both directions ----
  ...
  const boundary = async (name, from, act) => {
    ...
    console.log(`boundary ${name.padEnd(52)} ${before.id}:${before.step} → ${after.id}:${after.step}`);
  };
  await boundary('S16 end → S17 (forward, the legacy crossfade)', [0, 7],
    () => page.evaluate(() => window.__deck.advance()));
  await boundary('S17 b0 → S16 end (backward, the handoff)', [1, 0],
    () => page.evaluate(() => window.__deck.retreat()));
  await boundary('S17 end → S18 (forward, the legacy crossfade)', [1, 4],
    () => page.evaluate(() => window.__deck.advance()));
  await boundary('S18 b0 → S17 end (backward, the handoff)', [2, 0],
    () => page.evaluate(() => window.__deck.retreat()));
  await boundary('S18 end → S19 (forward, the legacy crossfade)', [2, 7],
    () => page.evaluate(() => window.__deck.advance()));
  await boundary('S19 b0 → S18 end (backward, the handoff)', [3, 0],
    () => page.evaluate(() => window.__deck.retreat()));
  ...
  const enterCold = async (slide, build) => {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        await page.goto(`${BASE}/?proto=act4&slide=${slide}`, { waitUntil: 'domcontentloaded' });
