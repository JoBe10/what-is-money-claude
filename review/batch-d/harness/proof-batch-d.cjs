// Batch D — the batch landed-state proof (the implementation brief §3).
//
// One claim, made checkable across the whole act IN THE DECK: EVERY SETTLED
// STATE OF SCENES 16–23, mounted cold through the real engine at its deck
// position (`?slide=<id>` — the engine's own deep link — plus the restored
// build step), IS ITS APPROVED CELL. Forty-two states, three comparisons
...
// 34-slide deck rather than the scratch route.
//
// Usage: node proof-batch-d.cjs [--port 5273]
...
const SCENES = [
  ['return-to-the-open-exchange', 16, 8],
  ['what-the-carrier-must-preserve', 17, 5],
  ['the-100-year-test', 18, 8],
  ['invert-the-question', 19, 2],
  ['how-the-carrier-can-fail-i', 20, 5],
  ['how-the-carrier-can-fail-ii', 21, 5],
  ['from-failure-to-requirement', 22, 3],
  ['the-comparison', 23, 6]
];
// [scene id, build, cell id]
const STATES = SCENES.flatMap(([id, n, beats]) =>
  Array.from({ length: beats }, (_, b) => [id, b, `s${n}-b${b + 1}`]));
...
(async () => {
  ...
  fs.writeFileSync(path.join(__dirname, '..', 'landed-proof-batch-d.json'), JSON.stringify({
    date: new Date().toISOString(),
    session: 'batch-d',
    rule: 'each of the 42 settled states of Scenes 16–23, mounted cold through the real engine at its deck position in the spliced 34-slide deck, is its approved cell (review/act-4/states — presenter-approved in full 3 Sep 2026) at zero differing pixels (channel delta > 2/255 counts). Comparisons: scene↔archived cell, fresh cell↔archived cell, scene↔fresh cell.',
