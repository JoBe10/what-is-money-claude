// Batch C implementation, Session 1 — the FAST-scope smoke
...
//   2. Route boot: `?proto=act3` runs the three scenes through the real
//      engine — 15 states.
...
//   6. Reduced-motion parity: the same 15 cold entries under
//      prefers-reduced-motion serialize IDENTICALLY to the settled
//      motion-on states (window.__act3's serializer) — checked
...
const SCENES = [
  { slide: 1, id: 'three-familiar-jobs', builds: 5 },
  { slide: 2, id: 'we-already-split-those-jobs', builds: 4 },
  { slide: 3, id: 'the-order-of-monetization', builds: 6 }
];
...
(async () => {
  ...
  const boundary = async (name, from, act) => {
    ...
    console.log(`boundary ${name.padEnd(44)} ${before.id}:${before.step} → ${after.id}:${after.step}`);
  };
  await boundary('S11 end → S12 (forward, the morph)', [0, 4],
    () => page.evaluate(() => window.__deck.advance()));
  await boundary('S12 b0 → S11 end (backward, the handoff)', [1, 0],
    () => page.evaluate(() => window.__deck.retreat()));
  await boundary('S12 end → S13 (forward, the authored cut)', [1, 3],
    () => page.evaluate(() => window.__deck.advance()));
  await boundary('S13 b0 → S12 end (backward, the handoff)', [2, 0],
    () => page.evaluate(() => window.__deck.retreat()));
  ...
  const enterCold = async (slide, build) => {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        await page.goto(`${BASE}/?proto=act3&slide=${slide}`, { waitUntil: 'domcontentloaded' });
