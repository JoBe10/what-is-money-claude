// Batch C — the capture strip, completed
...
//
// Usage: node capture-strip-batch-c.cjs [--port 5273]
...
const CELL_OF = [
  ...
  ['s14-b1', 's14-b2', 's14-b3', 's14-b4'],
  ['s15-b1-a', 's15-b2-a', 's15-b3-a', 's15-b4-a', 's15-b5-a', 's15-b6-a']
];
...
(async () => {
  ...
  await page.goto(`${BASE}/?slide=the-trade-off-keeps-moving`, { waitUntil: 'networkidle' });
  await page.waitForFunction(() => window.__deck, null, { timeout: 30000 });
  await page.evaluate(() => window.__deck._hideChrome());
  await settled();
  // Walk S10 to its end, then cross the splice seam into the act.
  for (let k = 0; k < 4; k += 1) { await page.evaluate(() => window.__deck.advance()); await settled(); }
  await page.evaluate(() => window.__deck.advance());
  await page.waitForTimeout(600);
  // The splice seam mid-flight: the act entered from Scene 10's end.
...
<html lang="en"><head><meta charset="utf-8"><title>Batch C — the capture strip (the act, complete)</title>
<style>
...
The walk enters across the splice seam from Scene 10 and plays all five scenes.</p>
<div class="row">
<figure><a href="./gesture-s11-entry.png"><img src="./gesture-s11-entry.png"></a><figcaption>gesture · the splice seam — the act entered from Scene 10's end</figcaption></figure>
