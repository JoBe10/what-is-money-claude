// R3 frame-capture proof (§C.1): navigation never manufactures black.
// Section 3's non-group slides open on authored black beats, so most of
// its boundaries belong to the authored-black class (they dim monotonically
// into designed darkness, never void-then-relight); the ladder group and
// the seam into Section 4 must show zero void frames.
//
...
//
// Usage: node flash-check-r3.cjs [--rm]
...
const SHOTS = path.join(OUT, 'screenshots', 'flash-frames');
...
const BOUNDARIES = [
  ...
  ['3-06 b4 → 3-07 lands on authored black', '3-06-what-your-money-is', 20, 4, 'ArrowRight', 'authored-black'],
...
(async () => {
  ...
  fs.writeFileSync(
    path.join(OUT, REDUCED ? 'flash-check-r3-results-rm.json' : 'flash-check-r3-results.json'),
