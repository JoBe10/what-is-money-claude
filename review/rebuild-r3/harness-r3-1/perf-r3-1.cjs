// R3.1 performance matrix: requestAnimationFrame delta collection across the
// section's heavier animation windows (all CSS transitions — no canvas
// work in Section 3). Usage: node perf-r3-1.cjs
...
const SCENARIOS = [
  ...
  ['ladder: thresholds brighten (3-03 b3→b4)', '3-03-the-order-of-monetization', 17, 3, 1300],
  ['tower completes + settle (3-06 b2→b3)', '3-06-what-your-money-is', 20, 2, 1600],
  ['tower tremor ripples upward (3-06 b3→b4)', '3-06-what-your-money-is', 20, 3, 1800],
  ['tower held question draws (3-06 b5→b6)', '3-06-what-your-money-is', 20, 5, 1900],
...
(async () => {
  ...
  fs.writeFileSync(path.join(OUT, 'performance-matrix-r3-1.txt'), lines.join('\n') + '\n');
  await browser.close();
})();
