...
//
// Usage: node register-r7-2.cjs [--port 4318]
...
(async () => {
  ...
  // The log's list. 2.05 is on it because the flagged gold arrival declares
  // `mixed` when its toggle is on — the gate must not fail the presenter for
  // ruling the candidate in.
  const DECLARED = [
  ...
  fs.writeFileSync(path.join(OUT, 'register-r7-2.json'), JSON.stringify({
    phase: 'R7.2', diagrams: DIAGRAMS, checks: results.length, failures: failures.length, rows, results
  }, null, 2));
  console.log(`\nR7.2 register audit: ${results.length} checks, ${failures.length} failures`);
  await browser.close();
