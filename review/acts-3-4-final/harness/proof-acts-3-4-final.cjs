// Acts III–IV final — the landed-state proof on the changed states (the Act V
// kickoff brief, Part A: "proofs on every changed state" before tag
// acts-3-4-final).
//
// One claim, made checkable IN THE DECK: every settled state of the scenes
// the four rulings touch — Scene 12 (the household frame retired; three
// beats), Scene 15 (the exit on the base slab), Scene 16's first beat (the
// homecoming, its entry now morphing from the slab) and Scene 23 (the
// table's header band) — mounted cold through the real engine at its deck
// position in the spliced 32-slide deck, IS ITS APPROVED CELL. Seventeen
// states, three comparisons each (scene↔archived cell, fresh cell↔archived,
// scene↔fresh cell), zero differing pixels (channel delta > 2/255 counts).
//
// The Act III cells build through review/act-3/harness/states.mjs and the
// Act IV cells through review/act-4/harness/states.mjs — each sheet's own
// record. s12-b2 and s12-b3 are the approved Argentina and principle cells
// carried under their new beats; building them fresh and matching at zero
// pixels proves the renumbering moved nothing but the beat. s15-b1-a …
// s15-b6-a and s23-b1 … s23-b4 were not re-rendered; proving them proves
// the rewirings moved nothing settled around the changes. s16-b1 proves the
// re-pointed entry still lands on the approved homecoming.
//
// Usage: node proof-acts-3-4-final.cjs [--port 5273]
const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');

const args = process.argv.slice(2);
const flag = (n, d) => { const i = args.indexOf(n); return i > -1 ? args[i + 1] : d; };
const PORT = flag('--port', '5273');
const BASE = `http://127.0.0.1:${PORT}`;
const REPO = path.join(__dirname, '..', '..', '..');

// [scene id, build, cell id, builder module, archive dir]
const A3 = ['/review/act-3/harness/states.mjs', 'review/act-3/states'];
const A4 = ['/review/act-4/harness/states.mjs', 'review/act-4/states'];
const STATES = [
  ['we-already-split-those-jobs', 0, 's12-b1', ...A3],
  ['we-already-split-those-jobs', 1, 's12-b2', ...A3],
  ['we-already-split-those-jobs', 2, 's12-b3', ...A3],
  ['the-tower', 0, 's15-b1-a', ...A3],
  ['the-tower', 1, 's15-b2-a', ...A3],
  ['the-tower', 2, 's15-b3-a', ...A3],
  ['the-tower', 3, 's15-b4-a', ...A3],
  ['the-tower', 4, 's15-b5-a', ...A3],
  ['the-tower', 5, 's15-b6-a', ...A3],
  ['the-tower', 6, 's15-b7', ...A3],
  ['return-to-the-open-exchange', 0, 's16-b1', ...A4],
  ['the-comparison', 0, 's23-b1', ...A4],
  ['the-comparison', 1, 's23-b2', ...A4],
  ['the-comparison', 2, 's23-b3', ...A4],
  ['the-comparison', 3, 's23-b4', ...A4],
  ['the-comparison', 4, 's23-b5', ...A4],
  ['the-comparison', 5, 's23-b6', ...A4]
];

const SEED_SCRIPT = () => {
  const SEED = 0x2545F491;
  let s = SEED >>> 0;
  Math.random = () => {
    s = (s * 1664525 + 1013904223) >>> 0;
    return s / 4294967296;
  };
};

(async () => {
  const browser = await chromium.launch();
  const errors = [];
  let page;
  const freshPage = async () => {
    if (page) await page.close();
    page = await browser.newPage({ viewport: { width: 1920, height: 1080 } });
    await page.addInitScript(SEED_SCRIPT);
    page.on('console', (m) => { if (m.type() === 'error') errors.push(m.text()); });
    page.on('pageerror', (e) => errors.push(String(e)));
  };
  await freshPage();

  const ready = async () => {
    await page.evaluate(async () => {
      await document.fonts.ready;
      await Promise.all([...document.querySelectorAll('img')].map((i) => i.decode().catch(() => {})));
    });
  };
  const settled = async () => {
    await page.waitForFunction(() => {
      if (window.__deck && window.__deck.transitioning) return false;
      for (const key of ['__act3', '__act4']) {
        const a = window[key];
        if (a && a.settled && !a.settled()) return false;
      }
      return document.getAnimations()
        .filter((an) => an.playState === 'running' || an.playState === 'pending').length === 0;
    }, null, { timeout: 40000 });
  };

  // ---- pass 1 · every cell, freshly rendered through its own sheet's pipeline
  const cellShots = {};
  let loaded = null;
  let n = 0;
  const loadBuilder = async (mod) => {
    await freshPage();
    await page.goto(`${BASE}/?proto=list`, { waitUntil: 'domcontentloaded' });
    await page.evaluate(async (m) => { window.__states = await import(m); }, mod);
    loaded = mod;
    n = 0;
  };
  for (const [, , id, mod] of STATES) {
    if (loaded !== mod || (n > 0 && n % 8 === 0)) {
      if (loaded) await page.evaluate(() => window.__states.teardown());
      await loadBuilder(mod);
    }
    n += 1;
    await page.evaluate((cellId) => window.__states.buildCell(cellId), id);
    await ready();
    await page.waitForTimeout(800);
    cellShots[id] = await page.screenshot({ clip: { x: 0, y: 0, width: 1920, height: 1080 } });
    console.log(`cell   ${id}`);
  }
  await page.evaluate(() => window.__states.teardown());

  // ---- pass 2 · the scenes cold at every build, at their deck positions -----
  const sceneShots = {};
  for (const [sceneId, build, id] of STATES) {
    await freshPage();
    await page.goto(`${BASE}/?slide=${sceneId}`, { waitUntil: 'networkidle' });
    await page.waitForFunction(() => window.__deck && !window.__deck.transitioning, null, { timeout: 30000 });
    const idx = await page.evaluate((sid) => window.__deck.slides.findIndex((s) => s.id === sid), sceneId);
    if (idx < 0) throw new Error(`${sceneId} is not in the deck`);
    await page.evaluate(([i, b]) => {
      sessionStorage.setItem('deck-state', JSON.stringify({ slideIndex: i, buildStep: b }));
    }, [idx, build]);
    await page.reload({ waitUntil: 'networkidle' });
    await page.waitForFunction(() => window.__deck, null, { timeout: 30000 });
    await ready();
    await settled();
    const at = await page.evaluate(() => ({ i: window.__deck.index, b: window.__deck.buildStep }));
    if (at.i !== idx || at.b !== build) throw new Error(`the deck landed at ${at.i}:${at.b}, expected ${idx}:${build}`);
    await page.evaluate(() => window.__deck._hideChrome());
    await page.waitForFunction(() => {
      const c = document.querySelector('.deck-chrome');
      return !c || getComputedStyle(c).opacity === '0';
    }, null, { timeout: 15000 });
    await page.waitForTimeout(150);
    sceneShots[id] = await page.screenshot({ clip: { x: 0, y: 0, width: 1920, height: 1080 } });
    console.log(`deck   ${sceneId}:${build}  ${id}`);
  }

  // ---- pass 3 · the pixel comparisons --------------------------------------
  await freshPage();
  await page.goto('about:blank');
  const diff = (bufA, bufB) => page.evaluate(async ([a, b]) => {
    const load = (src) => new Promise((res, rej) => {
      const i = new Image(); i.onload = () => res(i); i.onerror = rej; i.src = src;
    });
    const [ia, ib] = await Promise.all([load(a), load(b)]);
    const c = document.createElement('canvas'); c.width = 1920; c.height = 1080;
    const ctx = c.getContext('2d', { willReadFrequently: true });
    ctx.drawImage(ia, 0, 0);
    const da = ctx.getImageData(0, 0, 1920, 1080).data;
    ctx.clearRect(0, 0, 1920, 1080);
    ctx.drawImage(ib, 0, 0);
    const db = ctx.getImageData(0, 0, 1920, 1080).data;
    let diffPixels = 0; let maxDelta = 0;
    for (let p = 0; p < da.length; p += 4) {
      const d = Math.max(Math.abs(da[p] - db[p]), Math.abs(da[p + 1] - db[p + 1]), Math.abs(da[p + 2] - db[p + 2]));
      if (d > maxDelta) maxDelta = d;
      if (d > 2) diffPixels += 1;
    }
    return { diffPixels, maxDelta };
  }, [`data:image/png;base64,${bufA.toString('base64')}`, `data:image/png;base64,${bufB.toString('base64')}`]);

  const results = [];
  let failures = 0;
  for (const [sceneId, build, id, , dir] of STATES) {
    const archived = fs.readFileSync(path.join(REPO, dir, `${id}.png`));
    const sceneVsArchive = await diff(sceneShots[id], archived);
    const cellVsArchive = await diff(cellShots[id], archived);
    const sceneVsCell = await diff(sceneShots[id], cellShots[id]);
    const pass = sceneVsCell.diffPixels === 0 && sceneVsArchive.diffPixels === 0 && cellVsArchive.diffPixels === 0;
    if (!pass) failures += 1;
    results.push({ state: `${sceneId}:${build}`, cell: id, sceneVsArchive, cellVsArchive, sceneVsCell, pass });
    console.log(`${pass ? 'MATCH ' : 'FAIL  '} ${id.padEnd(12)} scene↔archive ${String(sceneVsArchive.diffPixels).padStart(7)}` +
      `  cell↔archive ${String(cellVsArchive.diffPixels).padStart(7)}  scene↔cell ${String(sceneVsCell.diffPixels).padStart(7)}`);
    if (!pass) {
      fs.writeFileSync(path.join(__dirname, `proof-fail-${id}-scene.png`), sceneShots[id]);
      fs.writeFileSync(path.join(__dirname, `proof-fail-${id}-cell.png`), cellShots[id]);
    }
  }

  fs.writeFileSync(path.join(__dirname, '..', 'landed-proof-acts-3-4-final.json'), JSON.stringify({
    date: new Date().toISOString(),
    session: 'acts-3-4-final',
    rule: 'each of the 17 settled states the four Part A rulings touch — S12 b1–b3, S15 b1–b7, S16 b1, S23 b1–b6 — mounted cold through the real engine at its deck position in the spliced 32-slide deck, is its approved cell at zero differing pixels (channel delta > 2/255 counts). Comparisons: scene↔archived cell, fresh cell↔archived cell, scene↔fresh cell. s12-b2 and s12-b3 prove the renumbering moved nothing but the beat; s15-b7 and s23-b5/b6 prove the two rewired treatments land on their re-rendered cells; the untouched cells prove the rewirings moved nothing settled around the changes; s16-b1 proves the re-pointed entry lands on the approved homecoming.',
    states: results.length,
    passed: results.length - failures,
    results,
    consoleErrors: errors
  }, null, 2));

  console.log(`\n${results.length - failures}/${results.length} states proven`);
  console.log(`console errors: ${errors.length}`);
  errors.forEach((e) => console.log('ERR', e));
  await browser.close();
  process.exit(failures || errors.length ? 1 : 0);
})();
