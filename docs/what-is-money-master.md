...
| 4 Sep 2026 | **Noted for the viewing, not ruled — Batch E: the fiat note's size in Scene 23's header band** (the Batch E implementation brief §1.3, recorded with the batch's two rulings). The act-5-states session reported that at the ruled 100 × 100 contain box the fiat note reads small, by the register's own framing of that render: the box caps both axes, which is the rails law's equal-weight form at the ruled height, so the near-16:9 note fills the width and stands 56 tall while a 4:5 portrait fills the height. **This is noted, not ruled: nothing changes on its account, and no session may act on it.** It is on the list to judge at the act viewing, beside the exit's two wiring numbers and Scene 16's entry, which stand as built by the same ruling. If the viewing decides the note should stand taller, the change is one number — the wide renders take up to 188 of width while the band's height stays at 100 — and it is the presenter's to make. |
