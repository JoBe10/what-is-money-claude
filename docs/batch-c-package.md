...
**Pipeline for this batch (in order):** ~~kickoff (staging + provenance sketch)~~ → ~~the §1 rulings recorded~~ → ~~scripts installed + maps frozen~~ → **the triad-anchored beat-state sheet** (`review/act-3/states/`, this session) → the presenter's flipbook pass → Batch C implementation (sessions under the sizing law) → the act viewing → exit test.
