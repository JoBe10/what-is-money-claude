...
**FROZEN 2 September 2026**, on the presenter's approval of the §2 scripts (ruling 4 of the Act III states brief: **S11 = 5 · S12 = 4 · S13 = 6 · S14 = 4 · S15 = 6 — Act III is 25 beats**). The beat count is the script's own `[→]` count. The presenter's essay voice pass remains the final authority over every word, and a word pass that changes an advance count amends these maps with it. **Amended 2 September 2026 (Batch C r2 ruling 3, master §13): S13 = 7** — the frozen map's merged two-gate advance splits at the legacy slide's own seam, one `[→]` inserted in §2 at zero word changes — **Act III is 26 beats.**
...
**S14 — The Coffee Objection** (4): the objection, and what it is actually about — one job, the middle one → the objection on the ladder: everyday payments come last → bitcoin's stage: everything points at store of value → then judge it there — what makes a good store of value?
**S15 — The Tower** (6): start where you live — the payment app → the app balance is a claim on your deposit → the deposit is a claim on base money, and the tower is wider than its base → fair in both directions: layers are how money scales, and layers inherit their base → the scoping: the question is the foundation asset → the held question — every layer is a claim on the layer below. *(The map's original "; the thread returns" staging clause is struck — r2 ruling 1, 2 Sep 2026, master §13. The beat count is unchanged.)*

# 2. Scripts [installed verbatim 2 September 2026; presenter's own pass comes later]

**Approved as drafted** (presenter, 2 September 2026) and installed from `docs/batch-c-scripts-draft.md` §-by-§, **mechanically diffed at zero word differences** — `review/act-3/harness/script-diff.cjs`. The S13 notes-armor block is installed notes-only, never spoken, exactly as marked.
...
[→] Before we can ask it properly, let me pull the rug slightly. This whole time I've said "money" — but is the thing in your bank account actually the base good we've been talking about? Start from the top, where you live: the payment app.
