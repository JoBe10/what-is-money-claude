...
| **S26-F4** the coexistence law | **PORT** | `4-20` build 4: *Bitcoin competes with their monetary function — not their reason to exist.* over the five assets. Serves S26 b9. **The falsifiability passage is spoken over this frame with no advance (row 2 ruled A, 4 Sep 2026)** |
...
| **S27-F1** one claim at the decision point | **PORT** — *row 1 ruled A, 4 Sep 2026* | `4-21-marginal-store-of-value-decision` build 0: the `ClaimObject` at focus at the decision point, nothing else; the entry line *"One claim, and five places it could go."* spoken over it. Serves S27 b1 at the default |
...
| **S28-F1** the fixed stock at rest | **PORT** — *row 4 ruled A, 4 Sep 2026* | `4-22-fixed-supply-reprices-at-margin` build 0 + `FixedSupplyField`: FIXED OUTSTANDING STOCK — the 7 × 5 `UnitGrid` from `UnitField`'s grammar (§0, the rhyme's source). No spoken advance of its own; the boundary from Scene 27 lands on it (the Act IV convention for a legacy build 0 with no entry line) |
| **S28-F2** the margin — the sliver actually for sale | **PORT** — *row 4 ruled A* | `4-22` build 1: *Demand does not need to absorb everything.*; the rightmost column lit as AVAILABLE AT THE MARGIN with its bracket; the three demand claims visible at 44 px, neutral. Serves S28 b1 |
| **S28-F3** the demand arriving | **PORT** — *row 4 ruled A* | `4-22` build 2: *It only needs to grow against a supply that cannot respond.*; the demand claims at focus, arriving at the margin. Serves S28 b2 |
...
**RULED A, 4 September 2026 (master §13).** The three entry lines are beats: S24 = 4 · S25 = 4 · S27 = 3; Act V is 27. S24-F1, S25-F1 and S27-F1 are PORT.
...
**RULED A, 4 September 2026 (master §13).** The passage is spoken over Scene 26's last frame, the coexistence law, with no advance; Scene 23 is not reopened.
...
**RULED A, 4 September 2026 (master §13).** The field is legacy `4-22`'s `FixedSupplyField`, ported whole; the Scene 28 cells are rendered from that implementation and the sheet's check proves the mounted grid against the grammar in the source.
...
**RULED 4 September 2026 (master §13; the Batch E implementation brief §1.2): the six seams are Batch E's to time, as their flags state.** Seams 1, 6 and 7 below, and the three the states sheet raised on its cells (Scene 26's internal merge, Scene 29's full clear, Scene 30's silence), are answered the same way: **the boundary is played as the legacy played it, the settled frame does not change, and nothing is redesigned.** In order — the Act IV → Scene 24 boundary is a crossfade; Scene 26's internal merge is the legacy crossfade; Scene 28's margin lights as the words land, its build 0 the composition the gesture starts from; the through-line's last journey is wired across the Scene 27 → Scene 28 boundary; Scene 29's full clear lands on the kicker alone; and Scene 30's silence is a scripted black beat with the retired wayline never shown. The remaining entries below are word-pass and legacy-fact records, and they stand as written.
