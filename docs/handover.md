...
2. **Prototype Gate 2 — the beat-state sheet awaits his eye** (tag `gate-2-states`): Jonas reviews `review/gate-2/states/sheet.html` cell by cell at full size and returns per-cell verdicts plus the D1–D8 selections (no bulk verdicts — the full-coverage rule). Then **motion r2** (Fable, top effort) rebuilds the prototype at 19 beats connecting approved states only, derivation banned; iterations tag `-r3`, … until his recorded word is *exceptional*. Iterate in the prototype, not in the deck.
