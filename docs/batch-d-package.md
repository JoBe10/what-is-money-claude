...
**Pipeline for this batch (in order):** ~~the kickoff brief~~ → ~~Part A: the Act III final rulings executed and proven (`act-3-final`)~~ → **this package and the provenance map (`act-4-foundation`)** → the presenter's one-pass ruling: the three ARGUABLE rows, the seven seam flags, the beat maps frozen → Session 2: the Act IV beat-state sheet (`act-4-states`) → the flipbook walk and the go-ahead → Batch D implementation → the act viewing.
...
**DERIVED 3 September 2026 — not yet frozen.** Each count is the sum of the legacy slides' own `[→]`s in the architecture's merge, at the default reading of the ARGUABLE rows (`docs/act-4-provenance.md` §2: the 4-01 lens retired; 4-06 closing Scene 16; the 4-15 lineup standing as its own frame). The presenter's rulings and his word pass freeze these maps; a ruling that changes a reading changes a count, and the table beneath says by how much. **Act IV is 43 beats at the default map.**
...
- **The Scene 4 return:** the architecture's canonical line, *"We left this exchange open. What has to survive until we close it?"*, opens S16 b1 verbatim; legacy 4-04's first paragraph follows it verbatim and uncut, with the seam flagged in place (§1.2, seam 2). Nothing was written to join them.
...
2. **Scene 22's count.** The architecture says three states; the scripts carry four advances. The package reads three compositions across four advances. If you meant three advances, two paragraphs merge in your word pass.
